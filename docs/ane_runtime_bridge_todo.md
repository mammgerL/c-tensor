...
1. Add deterministic correctness checks against baseline:
   - max abs diff threshold
   - failover to CPU fallback when out of bound
2. Add resource management for repeated compile/load cycles:
   - cap recompiles
   - explicit teardown hooks
3. Integrate ANE dense path into training loop behind explicit opt-in.
